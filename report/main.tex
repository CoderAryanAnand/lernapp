...
Hier interessierte uns, wie die Schüler:innen selbst zu diesen Themen stehen anstatt theoretisches Wissen.
Deshalb wurden die Fragen auf das Verhalten der Befragten gerichtet, wie beispielsweise ob sie oftmals Prüfungsstress erleben und weshalb, wie früh sie mit dem Lernen anfangen, ob sie regelmässig Pausen machen, etc.

Somit konnten wir mit der Entwicklung 
...
Die App richtet sich nach der Systemeinstellung (\texttt{prefers-color-scheme}). Ein früh geladenes Skript setzt die Klasse \texttt{dark} auf \texttt{<html>} bei dunkler Präferenz und entfernt sie sonst. Die Styles sind als Dualklassen (z.\,B. \texttt{bg-zinc-100} / \texttt{dark:bg-zinc-900}) angelegt. So ergibt sich eine konsistente Darstellung ohne separate Stylesheets und ohne sichtbare Umschalt­effekte.
...
\section{Tests}
Um die Qualität und Zuverlässigkeit der Kanti Koala Web-App sicherzustellen, wurden verschiedene Testfälle (funktional + Usability) durchgeführt. Die funktionalen Tests decken Backend-Logik und Frontend-Funktionalität ab (Authentifizierung, Agenda, Notenverwaltung, Lerntools, UI). Der formative Usability-Test (siehe eigenes Kapitel) evaluierte Bedienlichkeit, Verständlichkeit und Fehlertoleranz bei Kernaufgaben. Die vollständigen Testprotokolle der funktionalen Tests sind im Anhang dokumentiert.
