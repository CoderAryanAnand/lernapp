...
\JournalEntry[true]{Implementierung von Ganztags-Events und Notenorganisation}{3h (Aryan)}{Implementierung der "All day/multi day" Event-Funktionalität im Kalender. Einen Anfang für die Notenorganisation machen.}{Die Funktionalität für ganztägige und mehrtägige Events (`All day/multi day`) wurde implementiert. Ein Grundstein für die Notenorganisation wurde gelegt.}{Die `All day/multi day` Funktionalität ist implementiert, jedoch ist das Interface momentan nicht optimal ("nicht so schön").}
...
\JournalEntry[true]{Implementierung und Debugging des Lern-Algorithmus}{4h (Aryan)}{Der Algorithmus wurde implementiert. Dabei wurde viel Zeit für das Debugging des Algorithmus aufgewendet. Es wurden Funktionen für die Agenda-Einträge und Freizeit-Blöcke (`free\_gap`) erstellt. Probleme traten auf, weil beim Erstellen der Freiblöcke die Zeitzone nicht beachtet wurde.}{Der Lern-Algorithmus ist implementiert. Funktionen für Agenda-Einträge und das Finden von Freiblöcken sind integriert. Es wurden Fehler beim Umgang mit der Zeitzone behoben, welche zu falsch gesetzten Freiblöcken führten.}{Das Hauptproblem war die Timezone, da Python die Zeit mit der Local Time des Servers speichert, und das Frontend die Zeit der Local Time des Clients bekommt. Der Algorithmus selbst läuft nun und kann das Datum bestimmen und Lernblöcke eintragen.}

% 04.10.2025
\JournalDay{04.10.2025}
\JournalEntry[true]{Test und Feinabstimmung des Lern-Algorithmus}{3h (Aryan)}{Der Algorithmus wurde weiter getestet und angepasst. Ein kritischer Fehler, der dazu führte, dass die Freiblöcke von der falschen Seite des Tages ausgefüllt wurden, musste behoben werden. Die Logik, wie der Algorithmus die Zeit findet, musste korrigiert werden, sodass er die Zeit vom Anfang des Tages an sucht, anstatt vom Ende des Tages rückwärts zu arbeiten.}{Der Algorithmus ist nun funktionstüchtig und findet freie Zeitblöcke korrekt, um Lern-Events einzutragen. Die Logik zur Suche nach Freiblöcken am Anfang des Tages funktioniert nun wie gewünscht.}{Der anfängliche Fehler im Algorithmus, die Zeit von der falschen Seite des Tages zu füllen, ist behoben. Die Funktion des Algorithmus, das Datum und die Lernblöcke zu bestimmen, ist erreicht.}

% 05.10.2025
\JournalDay{05.10.2025}
\JournalEntry[true]{Algorithmus-Optimierung und Detail-Implementierung}{3h (Aryan)}{Das Frontend des Algorithmus wurde implementiert. Funktionen wie Benachrichtigungen und das Setzen der maximalen Zeit für den Algorithmus wurden hinzugefügt. Das Noten-Feature wurde ebenfalls implementiert.}{Das Frontend für den Lern-Algorithmus ist funktionstüchtig und benutzerfreundlich gestaltet. Das Noten-Feature ist implementiert. Benachrichtigungen und die Funktion zur Begrenzung der maximalen Lernzeit sind nun integriert.}{Die Implementierung des Noten-Features lief besser als erwartet. Der Algorithmus ist jetzt bereit für das Testing und die Optimierung der Parameter.}

% 06.10.2025
\JournalDay{06.10.2025}
\JournalEntry[true]{Implementierung der Notenorganisation und Umfrage-Auswertung}{Ca. 4.5h (Aryan: 3h, Simon: 1.5h)}{Aryan: Abschluss der Notenorganisation und Behebung kleinerer Bugs im Algorithmus. Das Noten-Feature wurde getestet, wobei ein Bug im Noten-Import auftrat und behoben wurde. Das Frontend für das Noten-Feature wurde optimiert. Simon: Die Umfrage-Auswertung wurde fortgesetzt, mit Fokus auf der Ausarbeitung des Abschnitts Lernmethoden.}{Aryan: Die Notenorganisation ist nun vollständig implementiert und getestet, inklusive der Behebung eines Bugs beim Noten-Import. Simon: Der Abschnitt zur Auswertung der Lernmethoden aus der Umfrage wurde fertiggestellt.}{Aryan: Die Programmierung lief besser als erwartet. Der Bug im Noten-Import war leicht zu beheben, nachdem er identifiziert wurde. Das Noten-Feature ist nun stabil. Simon: Die Umfrage-Resultate sind sehr nützlich, müssen aber noch mit den Interview-Ergebnissen abgeglichen werden, da es leichte Diskrepanzen gab.}
...
\JournalEntry[true]{Abschluss der Umfrage-Auswertung}{Ca. 1.5h (Simon)}{Die Umfrage-Analyse wurde abgeschlossen, indem einige Erkenntnisse \& ein paar konkrete Empfehlungen hinzugefügt wurden. Ebenso wurden die (seriösen) Tipps der Schüler:innen aus der Umfrage übertragen.}{Die Umfrage-Analyse ist nun fertiggestellt und kann so als Referenz gebraucht werden.}{Die Analyse der Umfrage begründet \& validiert unsere Web-Applikation sehr gut. Sie zeigt auf u.a. dass viele Schüler nur sehr spät mit dem Lernen anfangen, oft Prüfungsstress haben und auch an so einer Lernapp interessiert wären. Leider kam die fertiggestellte Analyse erst sehr spät, aber die Resultate waren auch ohne die formelle Analyse schon relativ sichtbar.}
